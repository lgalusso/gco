  ...
  <table width="600" border="0" align="center" cellpadding="0" cellspacing="0" class="tabela">
    <tr>
      ...
      <form id="form2" name="form2" method="POST" action="patrimonio/incluir_ajax.php<?=$frmActEdt?>" onsubmit="formSender(this, 'conteudo'); return false;"><fieldset>
        ...
        <table width="497" border="0" align="center" cellpadding="0" cellspacing="0" class="texto">
          ...
          <tr>
            <td><?=$r[1]?>* <?=$LANG['patrimony']['code']?><br />
              <input name="codigo" value="<?=$row[codigo]?>" type="text" class="forms" <?=$disable?> id="codigo" /></td>
            <td><?=$LANG['patrimony']['sector']?><br />
              <input name="setor" value="<?=$row[setor]?>" type="text" class="forms" <?=$disable?> id="setor" /></td>
          </tr>
          <tr>
            <td width="287"><?=$r[3]?>* <?=$LANG['patrimony']['description']?><br />
                <label>
                  <input name="descricao" value="<?=$row[descricao]?>" type="text" class="forms" <?=$disable?> id="descricao" size="45" maxlength="150" />
                </label>
                <label></label></td>
            <td width="210"><?=$LANG['patrimony']['value']?><br />
              <input name="valor" type="text" class="forms" <?=$disable?> id="valor" value="<?=$row[valor]?>" onKeypress="return Ajusta_Valor(this, event);" /></td>
          </tr>
          <tr>
            <td><?=$LANG['patrimony']['acquisition_date']?> <br />
              <input name="dataaquisicao" value="<?=$row[dataaquisicao]?>" type="text" class="forms" <?=$disable?> id="dataaquisicao" maxlength="150" onKeypress="return Ajusta_Data(this, event);" /></td>
            <td><?=$LANG['patrimony']['warranty_time']?><br />
              <input name="tempogarantia" type="text" class="forms" <?=$disable?> id="tempogarantia" value="<?=$row[tempogarantia]?>" /></td>
          </tr>
          <tr>
            <td><?=$LANG['patrimony']['color']?><br />
                <input name="cor" value="<?=$row[cor]?>" type="text" class="forms" <?=$disable?> id="cor" size="15" maxlength="50" /></td>
            <td><?=$LANG['patrimony']['quantity']?><br />
                <input name="quantidade" value="<?=$row[quantidade]?>" type="text" class="forms" <?=$disable?> id="quantidade" /></td>
          </tr>
          <tr>
            <td><?=$LANG['patrimony']['supplier']?><br />
                <input name="fornecedor" value="<?=$row[fornecedor]?>" type="text" class="forms" <?=$disable?> id="fornecedor" size="30" maxlength="50" />
              <br /></td>
            <td><?=$LANG['patrimony']['legal_document']?> <br />
              <input name="numeronotafiscal" value="<?=$row[numeronotafiscal]?>" type="text" class="forms" <?=$disable?> id="numeronotafiscal" /></td>
          </tr>
          <tr>
            <td valign="top"><?=$LANG['patrimony']['dimensions']?><br />
              <input name="dimensoes" value="<?=$row[dimensoes]?>" type="text" class="forms" <?=$disable?> id="dimensoes" /></td>
            <td><?=$LANG['patrimony']['comments']?><br />
              <textarea name="observacoes" cols="25" rows="5" class="forms" <?=$disable?> id="observacoes"><?=$row[observacoes]?></textarea></td>
          </tr>
          <tr>
            <td>&nbsp;</td>
            <td>&nbsp;</td>
		...
		<br />
        <div align="center"><br />
          <input name="Salvar" type="submit" class="forms" <?=$disable?> id="Salvar" value="<?=$LANG['patrimony']['save']?>" />
        </div>
      </form>      </td>
    </tr>
  </table>
