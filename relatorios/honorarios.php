...
?>
<p align="center"><font size="3"><b><?=$LANG['reports']['fee_table_report']?><br />
  <?=encontra_valor('convenios', 'codigo', $_GET['codigo_convenio'], 'nomefantasia')?></b></font></p><br />
...
?>
  <tr class="<?=$td_class?>" style="font-size: 11px">
    ...
    <td align="right"><?=$LANG['general']['currency'].' '.number_format($valor_particular, 2, ',', '.')?>
    </td>
    <td align="right"><?=$LANG['general']['currency'].' '.number_format($valor_convenio, 2, ',', '.')?>
    </td>
    <td align="right"><?=$LANG['general']['currency'].' '.number_format(($valor_particular - $valor_convenio), 2, ',', '.')?>
    </td>
    <td align="right"><?=@number_format((100 - ($valor_convenio / $valor_particular * 100)), 2, ',', '.')?> %
    </td>
  </tr>
<?
        $i++;
