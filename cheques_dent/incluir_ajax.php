  ...
  <table width="600" border="0" align="center" cellpadding="0" cellspacing="0" class="tabela">
    <tr>
      ...
      <form id="form2" name="form2" method="POST" action="cheques_dent/incluir_ajax.php<?=$frmActEdt?>" onsubmit="formSender(this, 'conteudo'); return false;"><fieldset>
        ...
        <table width="497" border="0" align="center" cellpadding="0" cellspacing="0" class="texto">
          ...
          <tr>
            <td width="287" height="40"><?=$LANG['check_control']['holder']?> <br />
                <label>
                  <input name="nometitular" value="<?=$row[nometitular]?>" <?=$disabled?> type="text" class="forms" id="nometitular" size="50" maxlength="80" />
                </label>
                <br />
              <label></label></td>
            <td width="210"><?=$r[2]?>* <?=$LANG['check_control']['value']?><br />
              <input name="valor" value="<?=$row[valor]?>" <?=$disabled?> type="text" class="forms" id="valor" onKeypress="return Ajusta_Valor(this, event);" /></td>
          </tr>
          <tr>
            <td height="40"><?=$LANG['check_control']['check_number']?><br />
              <input name="numero" value="<?=$row[numero]?>" <?=$disabled?> type="text" class="forms" id="numero" size="20" maxlength="150" /></td>
            <td><?=$LANG['check_control']['bank']?><br />
              <input name="banco" value="<?=$row[banco]?>" <?=$disabled?> type="text" class="forms" id="banco" /></td>
          </tr>
          <tr>
            <td height="40"><?=$LANG['check_control']['agency_number']?>:<br />
                <input name="agencia" value="<?=converte_data($row[agencia], 2)?>" <?=$disabled?> type="text" class="forms" id="agencia" size="20" maxlength="20" />
              <br /></td>
            <td><?=$LANG['check_control']['compensation_date']?>:<br />
                <input name="compensacao" value="<?=converte_data($row[compensacao], 2)?>" <?=$disabled?> type="text" class="forms" id="compensacao" size="14" maxlength="50" onKeypress="return Ajusta_Data(this, event);" />
              <br /></td>
          </tr>
          <tr>
            <td height="40"><?=$r[5]?>* <?=$LANG['check_control']['received_from']?>:<br />
                <input name="recebidode" value="<?=$row[recebidode]?>" <?=$disabled?> type="text" class="forms" id="recebidode" size="40" maxlength="50" />
              <br /></td>
            <td><?=$LANG['check_control']['forwarded_to']?>:<br />
                <input name="encaminhadopara" value="<?=$row[encaminhadopara]?>" <?=$disabled?> type="text" class="forms" id="encaminhadopara" size="40" maxlength="50" />
		...
		<br />
        <div align="center"><br />
          <input name="Salvar" type="submit" class="forms" id="Salvar" value="<?=$LANG['check_control']['save']?>" <?=$disabled?> />
        </div>
      </form>      </td>
    </tr>
  </table>
