...
$mainFolder    = 'fotos';   // folder where your albums are located - relative to root
$albumsPerPage = '12';       // number of albums per page
$itemsPerPage  = '3';       // number of images per page    
...
function print_pagination($numPages,$urlVars,$currentPage) {
   ...
   if ($numPages > 1) {
   		...
   		echo '<br/>';
	   echo 'Página '. $currentPage .' de '. $numPages;
	   echo '&nbsp;&nbsp;&nbsp;';
...
if (!isset($_GET['album'])) {
     ...
     if( count($albums) == 0 ) {
        ...
        if(isset($_GET['p'])) {
	     ...
	     for( $i=$start; $i<$start + $albumsPerPage; $i++ ) {
	        ...
	        if( isset($albums[$i]) ) {
				$fotos = "<a href=\"javascript:Ajax('pacientes/fotos','conteudo','album=".urlencode($albums[$i]) ."&codigo=".$_GET[codigo].$acao."')\">";
			    echo '<div class="thumb-album shadow">
				        
						<div class="thumb-wrapper">'.$fotos.'<span class="link_submenu_pacientes">'.
						...
						.$fotos.'<span class="link_submenu_pacientes">'.
   ...
   if ( count($files) == 0 ) {
      ...
      echo 'No hay fotos para el pacientes seleccionado.';
   ...
   for( $i=$start; $i<$start + $itemsPerPage; $i++ ) {
		  ...
		  if( isset($files[$i]) && is_file( $src_folder .'/'. $files[$i] ) ) { 
		  	...
		  	echo '&nbsp;';
	        echo '<img src="pacientes/'. $src_folder .'/thumbs/'. $files[$i] .'" width="'.$thumb_width.'" alt=""/>'; 
	        echo '&nbsp;';
     ...
     echo '<div align="center" class="paginate-wrapper">';
        ...
        $urlVars = "javascript:Ajax('pacientes/fotos','conteudo','codigo=".$_GET[codigo].$acao."&album=".urlencode($_GET['album'])."&amp;";
        print_pagination($numPages,$urlVars,$currentPage);
  
     echo '</div>';
    echo '<div  align="center" class="float-right">'.count($files).' imágenes en total</div>';
