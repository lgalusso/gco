...
?>
    <tr bgcolor="#<?=$odev?>" onmouseout="style.background='#<?=$odev?>'" onmouseover="style.background='#DDE1E6'">
      <td width="63%"><?=((encontra_valor('pacientes', 'codigo', $lista[$i]['codigo'], 'falecido') == 'Sim')?'<font color="#808080">':((em_debito($lista[$i][codigo]))?'<font color="red">':'')).$lista[$i][nome].' ('.encontra_valor('pacientes', 'codigo', $lista[$i]['codigo'], 'status').')'?></td>
      <td width="20%"><?=((encontra_valor('pacientes', 'codigo', $lista[$i]['codigo'], 'falecido') == 'Sim')?'<font color="#808080">':((em_debito($lista[$i][codigo]))?'<font color="red">':'')).$lista[$i][codigo]?></td>
      <td width="8%" align="center"><?=((verifica_nivel('pacientes', 'V'))?'<a href="javascript:Ajax(\'pacientes/incluir\', \'conteudo\', \'codigo='.$lista[$i][codigo].'&acao=editar\')"><img src="imagens/icones/editar.gif" alt="Editar" width="16" height="18" border="0"></a>':'')?></td>
      <td width="9%" align="center"><?=((verifica_nivel('pacientes', 'A'))?'<a href="javascript:Ajax(\'pacientes/gerenciar\', \'conteudo\', \'codigo='.$lista[$i][codigo].'" onclick="return confirmLink(this)"><img src="imagens/icones/excluir.gif" alt="Excluir" width="19" height="19" border="0"></a>':'')?></td>
    </tr>
<?
	}
?>
  ...
  <table width="750" border="0" align="center" cellpadding="0" cellspacing="0">
    <tr bgcolor="#<?=$odev?>" onmouseout="style.background='#<?=$odev?>'" onmouseover="style.background='#DDE1E6'">
      <td width="20%">
      <?=$LANG['patients']['total_patients']?>: <b><?=count($total_regs)?></b>
      </td>
...
?>
      ...
      <td width="43%" align="right">
        <img src="imagens/icones/imprimir.gif" border="0" weight="29" height="33"> <a href="relatorios/pacientes.php?sql=<?=ajaxurlencode($sql)?>" target="_blank"><?=$LANG['patients']['print_report']?></a>
        <img src="imagens/icones/etiquetas.gif" border="0"> <a href="etiquetas/print_etiqueta.php?sql=<?=ajaxurlencode($sql)?><?=($_GET['campo']=='nascimento' ? '&nasc=true' : '')?>" target="_blank"><?=$LANG['patients']['print_labels']?></a>
      </td>
    </tr>
  </table>
